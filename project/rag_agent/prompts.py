def get_conversation_summary_prompt() -> str:
    """
    Defines the persona and constraints for the 'summarize_history' node.
    This ensures the conversation context remains lean and focused on facts.
    """
...
def get_rewrite_query_prompt() -> str:
    """
    Defines the logic for the 'rewrite_query' node.
    This prompt instructs the LLM to perform query expansion, coreference resolution,
    and task decomposition (Map-Reduce).
    """
...
def get_orchestrator_prompt() -> str:
    """
    The 'System Message' for the actual retrieval agents.
    It defines exactly how the agent should utilize search tools and evaluate context.
    """
...
def get_fallback_response_prompt() -> str:
    """
    Defines the logic for the 'fallback_response' node.
    This is used when the search budget is exhausted or the LLM needs to synthesize
    a final answer from all retrieved fragments.
    """
...
def get_context_compression_prompt() -> str:
    """
    Defines the logic for the 'compress_context' node.
    This prompt instructs the LLM to summarize and deduplicate finding from 
    multiple search iterations to keep the context window manageable.
    """
...
def get_aggregation_prompt() -> str:
    """
    Defines the logic for the 'aggregate_answers' node.
    This prompt is the final 'Reduce' phase of the Map-Reduce workflow,
    weaving parallel research threads into a single natural-language reply.
    """
