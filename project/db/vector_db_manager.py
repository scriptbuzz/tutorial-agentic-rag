...
class VectorDbManager:
    """
    Manages the connection and operations for the Qdrant vector database.
    It handles the creation of collections with 'Hybrid Search' configurations,
    simultaneously supporting Dense (semantic) and Sparse (keyword) vectors.
    """
    __client: QdrantClient
    __dense_embeddings: HuggingFaceEmbeddings
    __sparse_embeddings: FastEmbedSparse
    def __init__(self):
        # Uses a local SQLite-backed Qdrant instance for storage
        self.__client = QdrantClient(path=config.QDRANT_DB_PATH)
        
        # Initialize Dense embeddings: semantic search powered by Sentence-Transformers
        self.__dense_embeddings = HuggingFaceEmbeddings(model_name=config.DENSE_MODEL)
        
        # Initialize Sparse embeddings: keyword-based search powered by BM25
    ...
    def get_collection(self, collection_name) -> QdrantVectorStore:
        """
        Wraps the client and embeddings into a LangChain-compatible VectorStore object.
        This enables 'Hybrid' retrieval where dense and sparse scores are fused.
        """
        try:
