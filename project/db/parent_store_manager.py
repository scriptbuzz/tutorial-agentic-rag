...
class ParentStoreManager:
    """
    Manages the persistent storage of 'Parent' document chunks on the local filesystem.
    Parent chunks provide the large-scale context needed by the LLM to generate
    accurate answers after the vector search has found the relevant 'Child' snippet.
    """
    __store_path: Path
    ...
    def save(self, parent_id: str, content: str, metadata: Dict) -> None:
        """
        Saves a single parent chunk to a flat JSON file on disk.
        Since these are large documents (~3000 tokens), we keep them 
        off the vector DB to save memory and only fetch by ID when needed.
        """
    ...
    def load(self, parent_id: str) -> Dict:
        """Loads a raw JSON file from the store by its identifier."""
    ...
    def _get_sort_key(id_str):
        """
        Extracts the numerical suffix from IDs (e.g., 'doc_parent_5' -> 5)
        to ensure multi-chunk documents are retrieved in the correct logical order.
        """
