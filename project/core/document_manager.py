...
class DocumentManager:
    """
    Handles the ingestion, conversion, and indexing of documents.
    It splits raw PDFs into markdown, processes them through the Chunker
    to establish the Parent-Child hierarchy, and inserts the embeddings
    into Qdrant while saving the parent chunks to disk.
    """
    ...
    def add_documents(self, document_paths, progress_callback=None):
        ...
        for i, doc_path in enumerate(document_paths):
            ...
            try:            
                # 1. Convert to Markdown. Vectors represent text structure much better when formatted as MD.
                if Path(doc_path).suffix.lower() == ".md":
                    shutil.copy(doc_path, md_path)
                else:
                    pdfs_to_markdowns(str(doc_path), overwrite=False)            
                
                # 2. Execute Hierarchical Splitting
                # The returned child_chunks are small (~500 tokens) and perfectly sized
                # for semantic dense search retrieval.
                # The parent_chunks are large (~3000 tokens) and linked to the children via metadata IDs.
                ...
                # 3. Store Child Chunks in the Vector Database
                # This triggers the embedding model to convert the text to dense vectors.
                collection = self.rag_system.vector_db.get_collection(self.rag_system.collection_name)
                collection.add_documents(child_chunks)
                
                # 4. Store Parent Chunks locally
                # These are NOT embedded; they are just JSON files used to retrieve context efficiently.
